...
namespace Tests.Items
{
  public class StorageContainerTests
  {
    ...
    private IItemModel _model1;
    private IItemModel _model3;
    ...
    public void Setup()
    {
      ...
      _model1 = Mock.Of<IItemModel>(model => model.StackAmount == 10 && model.Name == "Model 1");
      _model2 = Mock.Of<IItemModel>(model => model.StackAmount == 20 && model.Name == "Model 2");
      _model3 = Mock.Of<IItemModel>(model => model.StackAmount == 30 && model.Name == "Model 3");
    ...
    {
      Assert.AreEqual(new InventoryStack(_model1, 5), Get(5));
    }
    ...
    public void Adding_fills_existing_first()
    {
      ...
      Assert.AreEqual(new InventoryStack(_model1, 10), Get(5));
    }
    ...
    public void Cannot_exceed_model_limit()
    {
      ...
      // still should be ten
      Assert.AreEqual(new InventoryStack(_model1, 10), stack);

      // with the remaining added to the third
      Assert.AreEqual(new InventoryStack(_model1, 0), remaining);
      Assert.AreEqual(new InventoryStack(_model1, 5), Get(2));
    }
    ...
    public void Cursor_fill_returns_any_extras()
    {
      ...
      Assert.AreEqual(new InventoryStack(_model1, 10), Get(5));
      Assert.AreEqual(new InventoryStack(_model1, 5), remaining);
    }
    ...
    {
      Assert.AreEqual(InventoryStack.Empty, Get(2));

      var remaining = _container.Put(_container.GetCursor(2), new InventoryStack(_model1, 5));

      Assert.AreEqual(new InventoryStack(_model1, 5), Get(5));
      Assert.AreEqual(InventoryStack.Empty, remaining);
    }
    ...
    public void Put_in_a_non_empty_slot_returns_the_original_amount()
    {
      ...
      Assert.AreEqual(contentsOfSlotZero, Get(0));
    ...
    public void Removing_a_stack_removes_those_items()
    {
      ...
      // is now empty
      Assert.AreEqual(InventoryStack.Empty, Get(5));
      // and we returned what we removed
      Assert.AreEqual(new InventoryStack(_model1, 5), removed);
    }
  }
}